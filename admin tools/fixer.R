fixer <- function(tableRow, newValue_phase, newValue_task, newValue_detail, newValue_invalid) {
  ...
  df = run_archive[tableRow,] %>% 
    select(date, rat_name, rat_ID, assignment, invalid) %>% unnest_wider(assignment)
  ...
  if(newValue_invalid != run_archive[tableRow,]$invalid) run_archive[tableRow,]$invalid = newValue_invalid
