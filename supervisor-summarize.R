...
InitializeWriter <- function() {
      ...
      Build_Counts <- function() {
        ...
        if (phase_current == "Tones" & task_current %in% c("Training", "Reset") & pre_HL) {
          count_df = rat_runs %>%
            ...
            group_by(task, detail) %>%
            reframe(task = unique(task), detail = NA,
                      date = tail(date, 1), n = n(),
                      condition = "baseline")
        }
        ...
        if (phase_current %in% c("BBN", "Tones") & task_current %in% c("Rxn", "TH") & !pre_HL) {
          ...
          BBN_counts = df %>%
            ...
            reframe(task = paste("BBN", unique(task)), detail = unique(detail),
                      date = tail(date, 1), n = n(),
          ...
          Tones_counts = df %>%
            ...
            group_by(task, detail, condition) %>%
            reframe(task = paste("Tones", unique(task)), detail = unique(detail),
                      date = tail(date, 1), n = n(),
                      condition = unique(condition)) %>%
            dplyr::arrange(condition, task)
        ...
        if (phase_current == "Tones" & task_current %in% c("Training", "Reset") & pre_HL & detail_current == "Oddball") {
          count_df = rat_runs %>%
            ...
            group_by(task, detail) %>%
            reframe(task = unique(task), detail = detail_current,
                      date = tail(date, 1), n = n(),
                      condition = "baseline")
        }
        ...
        if (experiment_current == "Oddball Training") {
          count_df = rat_runs %>%
            ...
            reframe(task = unique(task), frequency = unique(frequency), detail = unique(detail),
                      date = tail(date, 1), n = n(),
                      condition = NA) %>%
  ...
  if(exists("custom_rats")) {
    writeLines("Processing custom rats ONLY")
    file_name = "supervisor-custom.xlsx"
    custom_rats %>% lapply(Add_Rat_To_Workbook)
  } else {
    file_name = "supervisor.xlsx"
    rat_archive %>%
